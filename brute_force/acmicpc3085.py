...
for r in range(N):
    for c in range(N):
        if c < N-1 and board[r][c] != board[r][c+1]:
            ...
            max_cnt = max(max_cnt, count_longest_column(board[r][c], c), count_longest_column(board[r][c+1], c+1))
            board[r][c], board[r][c+1] = board[r][c+1], board[r][c]
        if r < N-1 and board[r][c] != board[r+1][c]:
            board[r][c], board[r+1][c] = board[r+1][c], board[r][c]
            max_cnt = max(max_cnt, count_longest_row(board[r][c], r), count_longest_row(board[r+1][c], r+1))
            board[r][c], board[r+1][c] = board[r+1][c], board[r][c]
